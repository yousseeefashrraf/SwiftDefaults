...
public final class UserDefaultsManager: @unchecked Sendable {
    ...
    public func initiate<T: DefaultValue>(forType type: T.Type) where T.defaultType: RawRepresentable, T.defaultType.RawValue == String{
    ...
    ///
    public func isStored(forKey key: String) -> Bool{
    ...
    ///
    public func store<T: DefaultValue>(forType type: T.Type,value: T) where T: RawRepresentable, T.RawValue == String {
        if isStored(forKey: type.key) {
            UserDefaults.standard.setValue(value.rawValue, forKey: type.key)
        }
    }
    
    public func getStoredValue<T: DefaultValue>(forType type: T.Type)  -> String? where T.defaultType: RawRepresentable, T.defaultType.RawValue == String{
